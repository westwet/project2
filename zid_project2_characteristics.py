...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    def check_size_std(group):
        ...
        if group_size < 18:
            ...
            return group.std()
    df_return = ret[ret_freq_use[0]]
    df_return.index = df_return.index.to_period('M')

    col_name = df_return.columns
    # 每一列单独进行处理，防止出现两列日期不一致的情况
    temp = []
    for i in col_name:
        temp.append(df_return[i].groupby(df_return.index).apply(check_size_std))

    #df_vol = df_return.groupby(df_return.index).apply(check_size_std)
    df_vol = pd.concat(temp, axis=1, join='outer')
    df_vol.columns = [i+'_{}'.format(cha_name) for i in df_vol.columns]# 波动率表示
    df_vol.index.name = 'Year_Month' # 设置index_name
...
def merge_tables(ret, df_cha, cha_name):
    ...
    """
    # <COMPLETE THIS PART>
    return_df = ret['Monthly']
    cha_col = [i+'_{}'.format(cha_name) for i in return_df.columns]#
    df_concat = pd.concat([return_df, df_cha], axis=1, join='outer')
    for i in cha_col:
        df_concat[i] = df_concat[i].shift(1)
    return df_concat
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    """
    # <COMPLETE THIS PART>
    _test_vol_input_sanity_check(ret, cha_name, ret_freq_use)
    vol_df = vol_cal(ret,cha_name,ret_freq_use)
    final_df = merge_tables(ret,vol_df,cha_name)
    return final_df
...
if __name__ == "__main__":
    ...
    # made_up_ret_dict = _test_ret_dict_gen()
    #made_up_ret_dict = etl._test_aj_ret_dict(tickers=['AAPL', 'TSLA'], start='2010-05-15', end='2010-08-31')
