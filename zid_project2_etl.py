...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    #return_df = df.loc[start:end][prc_col]
...
def monthly_return_cal(prc):
    ...
    prc_temp = prc.index.to_period('M')
    # print("展示")
    # print(prc_temp)
    label = prc_temp.value_counts()>=18
    date_take = label[label].index

    # 计算得到月收益率
    convert_prc = prc.resample('M').last() # 把日周期变为月周期，并以这个月最后一个数据作为它的数据
    convert_prc.index = convert_prc.index.to_period('M')
    return_df = convert_prc.pct_change(1)
    return_df = return_df.dropna()
    return_df = return_df.loc[date_take]
    return return_df
...
def aj_ret_dict(tickers, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    all_daily = [] # 接收日收益率的series
    all_monthly = [] # 接收月收益率的series
    for tick in tickers:
        prc = read_prc_csv(tick, start,end)
        all_daily.append(daily_return_cal(prc))
        all_monthly.append(monthly_return_cal(prc))

    day_df = pd.concat(all_daily,axis=1,join='outer')
    day_df.columns = [i.lower() for i in tickers]
    day_df.index.name = 'Date'

    month_df = pd.concat(all_monthly, axis=1, join='outer')
    month_df.columns = [i.lower() for i in tickers]
    month_df.index.name = 'Year_Month'
    return {'Daily':day_df,'Monthly':month_df}
